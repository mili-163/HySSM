...
def parse_data(raw_text):
    ...
    for line in lines:
        # 1. 预处理：删除斜杠周围的所有空格，防止 "79.5 / 79.3" 被切割成两部分
        # 将 " /" 和 "/ " 替换为 "/"
        ...
        values = []
        for p in parts[1:]:
            # 2. 安全转换：过滤掉空字符串，防止 float('') 报错
            try:
                # 过滤掉 split('/') 产生的空项，例如 '/79.3' -> ['', '79.3']
...
# plt.subplots_adjust(bottom=0.15, wspace=0.25, hspace=0.15)
...
#            ncol=8, fontsize=11, frameon=True, edgecolor='black')
...
fig, axes = plt.subplots(2, 3, figsize=(18, 10), sharex=True)
# 调整间距：增加 bottom 以容纳更大的图例，减小 hspace 让两行图更紧凑
...
# 修复：使用 get_lines() 替代 legendHandles，兼容性更好
for handle in lgd.get_lines():
    handle.set_markersize(10) # 增大图例中的标记点
