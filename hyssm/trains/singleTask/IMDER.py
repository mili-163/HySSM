...
class IMDER():
    ...
    def do_train(self, model, dataloader, return_epoch_results=False, model_save_dir="", resume_from_checkpoint=False):
        ...
        if resume_from_checkpoint:
            model_load_path = f"{model_save_dir}/imder-{self.args.dataset_name}.pth"
            origin_model = torch.load(model_load_path)
        ...
        while True:
            ...
            with tqdm(dataloader['train']) as td:
                for batch_data in td:
                    ...
                    # forward
                    zs = {k: v.to(self.args.device) for k, v in batch_data['zs'].items()}
                    zp = {k: v.to(self.args.device) for k, v in batch_data['zp'].items()}
                    model_kwargs = {'zs': zs, 'zp': zp}
                    ...
                    if miss_two / (np.round(len(dataloader['train']) / 10) * 10) < miss_2[int(self.args.mr*10-1)]:  # missing two modal
                        outputs = model(text, audio, vision, num_modal=1,**model_kwargs)
                        miss_two += 1
                    elif miss_one / (np.round(len(dataloader['train']) / 10) * 10) < miss_1[int(self.args.mr*10-1)]:  # missing one modal
                        outputs = model(text, audio, vision, num_modal=2,**model_kwargs)
                        miss_one += 1
                    else:  # no missing
                        outputs = model(text, audio, vision, num_modal=3,**model_kwargs)
            ...
            model_save_path = f"{model_save_dir}/epoch{epochs}.pth"
