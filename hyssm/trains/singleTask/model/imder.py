...
class IMDER(nn.Module):
    def __init__(self, args):
        ...
        LLM_DIM = 3584
        self.score_l = ScoreNet(self.marginal_prob_std_fn, channels=[32, 64, 128, 256], zs_dim=LLM_DIM, zp_dim=self.orig_d_l) # Text Zp Dim
        self.score_v = ScoreNet(self.marginal_prob_std_fn, channels=[32, 64, 128, 256], zs_dim=LLM_DIM, zp_dim=self.orig_d_v) # Video Zp Dim
        self.score_a = ScoreNet(self.marginal_prob_std_fn, channels=[32, 64, 128, 256], zs_dim=LLM_DIM, zp_dim=self.orig_d_a) # Audio Zp Dim
    ...
    def forward(self, text, audio, video, num_modal=None, zs=None, zp=None, force_modal_idx=None):
        ...
        zs_l, zp_l = zs['text'], zp['text']
        zs_a, zp_a = zs['audio'], zp['audio']
        zs_v, zp_v = zs['visual'], zp['visual']
