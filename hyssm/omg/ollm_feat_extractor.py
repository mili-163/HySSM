...
class OmniFeatureExtractor:
    def __init__(self, model_path, device="cuda"):
        ...
        # 加载模型
        # 注意：这里根据实际模型类名修改，通常 Qwen2.5-VL 或 Omni 使用 Qwen2_5_VLForConditionalGeneration
        # 如果你的环境里确切是 Qwen2_5OmniThinkerForConditionalGeneration，请替换回来
    ...
    def get_text_embedding(self, text: str) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            print(f"Text embedding shape: {last_hidden_state.shape}")
    ...
    def construct_prompt(self, item):
        ...
        print("v/a path check:", item['visual'], item['audio'])

        if item['visual'] is None:
            print(f"Visual path is None for item {item['key']}")
            exit(0)
        if item['audio'] is None:
            print(f"Audio path is None for item {item['key']}")
            exit(0)
    ...
    def process_item(self, item):
        ...
        if conversation is None:
            print(f"Skipping {item['key']}: No valid modalities.")
            return None
...
def main():
    ...
    for key, item in tqdm(data_items.items()):
        ...
        print(f"Processed item {key}, path: {item.get('visual','N/A')}, {item.get('audio','N/A')}")
        if result:
