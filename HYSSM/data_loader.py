...
class MMDataset(Dataset):
    ...
    def __init_mosi(self):
        ...
        possible_paths = [
            os.getenv("MOSI_DATA_PATH", ""),
            "/root/FoldFish/dataset/CoraDiff-Dataset/MOSI/mosi_unified.pt",
            str(Path(__file__).parent.parent / "dataset" / "MOSI" / "mosi_unified.pt"),
            str(Path(__file__).parent.parent / "dataset" / "mosi_unified.pt"),
        ]
        ...
        if pt_path is None:
            pt_path = possible_paths[1]  # 使用原始路径，让错误信息更清晰
        ...
        self.global_zs_lookup = {}   # Semantic states from LLM
        self.global_phys_lookup = {} # Signal space features
        ...
        for k, v in data_dict.items():
            ...
            # Signal space features: Global Mean Pooling [SeqLen, Dim] -> [Dim]
        ...
        for video_id in self.data_list:
            ...
            self.samples[video_id] = {
                ...
                'retrieval': item['retrieval'] # Evidence retrieval: neighbor keys and scores
            }
    ...
    def __init_mosei(self):
        ...
        possible_paths = [
            os.getenv("MOSEI_DATA_PATH", ""),
            "/root/FoldFish/dataset/CoraDiff-Dataset/MOSEI/mosei_unified.pt",
            str(Path(__file__).parent.parent / "dataset" / "MOSEI" / "mosei_unified.pt"),
            str(Path(__file__).parent.parent / "dataset" / "mosei_unified.pt"),
        ]
    ...
    def __init_chsims(self):
        ...
        possible_paths = [
            os.getenv("CHSIMS_DATA_PATH", ""),
            "/root/FoldFish/dataset/CoraDiff-Dataset/CH-SIMS/ch_sims_unified.pt",
            str(Path(__file__).parent.parent / "dataset" / "CH-SIMS" / "ch_sims_unified.pt"),
            str(Path(__file__).parent.parent / "dataset" / "ch_sims_unified.pt"),
        ]
    ...
    def __getitem__(self, index):
        ...
        # --- C. Build Signal Space Features (via Evidence Retrieval) ---
        my_phys = self.global_phys_lookup[video_id] # Use own features as fallback
        sample['zp'] = {}
        
        for m in ['text', 'audio', 'visual']:
            ret_info = item['retrieval'].get(m, None)
            
            # If no retrieval results, use own features (Self-Prior)
            if ret_info is None or len(ret_info['keys']) == 0:
                ...
                weights = F.softmax(scores, dim=0) # Normalize scores
                
                neighbor_feats = []
                for k in neighbor_keys:
                    # Retrieve neighbor's signal space features
                ...
                # Weighted aggregation: [K] @ [K, Dim] -> [Dim]
